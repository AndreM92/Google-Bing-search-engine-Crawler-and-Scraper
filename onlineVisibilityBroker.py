...
def crawlScrape():
    ...
    for k in keywords:
        ...
        while page <= 5:
            ...
            # Code block for the collection of google hits
            ...
            for h in hits:
                ...
                for c in companies:
                    if c + '.de' in link or c + '.com' in link or c + '.net' in link or 'de.' + c in link:
                        ...
                        dataHits.append([c,k,rankHits,page,title,content,link])
...
dfHits = pd.DataFrame(data[1],columns=['company','keyword','rank','page','title','content','link'])
